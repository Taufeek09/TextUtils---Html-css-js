...
function dis() {
    ...
    wordCountElement.textContent = charCount + " Characters and " + wordCount + " Words";
    let paragraphCount = text.split(/\n\s*\n/).filter(paragraph => paragraph.trim() !== '').length;
    let sentenceCount = text.split(/[.!?]+/).filter(sentence => sentence.trim() !== '').length;
    sentenceCountElement.textContent = sentenceCount + " Sentences and " + paragraphCount + " Paragraphs";;
    let questionCount = (text.match(/\?/g) || []).length;
    let exclamationCount = (text.match(/\!/g) || []).length;
    questionCountElement.textContent = questionCount + " Questions and " + exclamationCount + " Exclamations";
    let wordsPerMinute = 100;
    let estimatedMinutes = Math.ceil(wordCount / wordsPerMinute);
    minReadElement.textContent = "Estimated Reading Time: " + estimatedMinutes + " min";
}
...
function TextUnicode() {
    let textBox = document.getElementById("mybox");
    textBox.value = [...textBox.value].map(c => c.charCodeAt(0).toString(16)).join(' ');
    alert("Text Converted to Unicode", "1500");
}

function textToSentenceCase() {
    let textBox = document.getElementById("mybox");
    textBox.value = textBox.value.toLowerCase().replace(/(^\s*\w|[\.\!\?]\s*\w)/g, function (char) {
        return char.toUpperCase();
    });
